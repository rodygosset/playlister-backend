...
def search_songs(db: Session, search_params: schemas.SongSearchParams, skip: Optional[int] = None, max: Optional[int] = None) -> List[models.Song]:
    """
    ...
    if search_params.duration is not None:
        songs = songs.filter(models.Song.duration == search_params.duration)
    ...
    if search_params.genres is not None:
        for genre in search_params.genres:
            db_genre = crud.get_genre_by_name(db, genre)
            if db_genre is None:
    ...
    if search_params.artists is not None:
        for artist in search_params.artists:
            db_artist = crud.get_artist_by_name(db, artist)
