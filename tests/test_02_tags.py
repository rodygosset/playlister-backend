...
def test_post_tag_401_login():
    """
    Make sure that a POST at '/api/tags/':
    - returns an HTTP 401 when the user isn't logged in.
    """
    global post_data
    # trying to post without being logged in
    response = client.post("/api/tags/", json=post_data)
...
def test_post_tag():
    """
    Make sure that a POST at '/api/tags/':
    - returns the newly created 'Tag' object when nothing goes wrong.
    """
    global post_data
    # log in as the test user
    auth_header = get_auth_header(login_as_test(client))
    # trying to post without running into any HTTP errors
    response = client.post("/api/tags/", headers=auth_header, json=post_data)
...
def test_post_tag_409_duplicate():
    """
    Make sure that a POST at '/api/tags/':
...
def test_get_all_tags_401_login():
    """
    Make sure that a GET at '/api/tags/':
    - returns an HTTP 401 when the user isn't logged in.
    """
...
def test_get_all_tags_ownership():
    """
    Make sure that a GET at '/api/tags/':
    ...
    # when making this request from an account that hasn't created any tags
    # ==> users can't access each other's music libraries
    response = client.get("/api/tags/", headers=auth_header)
    assert response.status_code == 200, response.text
    data = response.json()
...
def test_get_all_tags():
    """
    Make sure that a GET at '/api/tags/':
    - returns the list of 'Tag' objects when nothing goes wrong.
    """
    # logging in as the test user 
    auth_header = get_auth_header(login_as_test(client))
...
def test_put_tag_401_login():
    """
    Make sure that a PUT at '/api/tags/{name}':
    - returns an HTTP 401 when the user isn't logged in as an admin, or at all.
    """
    global put_data
    # try to PUT without being logged in
    response = client.put("/api/tags/Relaxing", json=put_data)
    assert response.status_code == 401, response.text


def test_put_tag_404_ownership():
    """
    Make sure that a PUT at '/api/tags/{name}':
    ...
    auth_header = get_auth_header(login_as_test1(client))
    response = client.put("/api/tags/Relaxing", headers=auth_header, json=put_data)
    assert response.status_code == 404, response.text

def test_put_tag_404_name():
    """
    Make sure that a PUT at '/api/tags/{name}':
    - returns an HTTP 404 when the 'Tag' object doesn't exist in the DB.
    """
    global put_data
    # log in as the test user
    auth_header = get_auth_header(login_as_test(client))
    # try to PUT an item that doesnt exist
    response = client.put("/api/tags/non_existent_item", headers=auth_header, json=put_data)
    assert response.status_code == 404, response.text

def test_put_tag_409_name_in_use():
    """
    Make sure that a PUT at '/api/tags/{name}':
    - returns an HTTP 409 when the 'new_name' attribute corresponds to an existing Tag object.
    """
    global put_data
    # log in as the test user
    auth_header = get_auth_header(login_as_test(client))
    ...
    # ==> 409 
    response = client.put("/api/tags/Relaxing", headers=auth_header, json={"new_name": "Vacation Vibes"})
    assert response.status_code == 409, response.text

def test_put_tag():
    """
    Make sure that a PUT at '/api/tags/{name}':
    - returns the updated 'Tag' object when nothing goes wrong.
    """
    global put_data
    # log in as the test user
    auth_header = get_auth_header(login_as_test(client))
...
def test_delete_tag_401_login():
    """
    Make sure that a DELETE at '/api/tags/{name}':
    - returns an HTTP 401 when the user isn't logged in as an admin, or at all.
    """
    # try to DELETE without being logged in
    response = client.delete("/api/tags/Energetic")
    assert response.status_code == 401, response.text


def test_delete_tag_404_ownership():
    """
    Make sure that a DELETE at '/api/tags/{name}':
    - returns an HTTP 404 when the user tries to access data located in another user's library.
    """
    # log in as the test1 user
    auth_header = get_auth_header(login_as_test1(client))
    response = client.delete("/api/tags/Energetic", headers=auth_header)
    assert response.status_code == 404, response.text


def test_delete_tag_404():
    """
    Make sure that a DELETE at '/api/tags/{name}':
    - returns an HTTP 404 when the 'Tag' object doesn't exist in the DB.
    """
    ...
    auth_header = get_auth_header(login_as_test(client))
    # try to PUT an item that doesnt exist
    response = client.delete("/api/tags/non_existent_item", headers=auth_header)
    assert response.status_code == 404, response.text

def test_delete_tag():
    """
    Make sure that a DELETE at '/api/tags/{name}':
    - returns the deleted 'Tag' object when nothing goes wrong.
    """
    # log in as the test user
    auth_header = get_auth_header(login_as_test(client))
